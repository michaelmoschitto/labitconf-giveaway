...
const RETURNING_USER_KEY = "mezo_returning_user";

export const EntryFormMobile = ({ dict, lang }: EntryFormComponentProps) => {
  const [showForm, setShowForm] = useState(false);

  // Check if user is returning from Mezo wallet creation
  useEffect(() => {
    const checkReturningUser = () => {
      if (typeof window !== "undefined") {
        const isReturningUser = localStorage.getItem(RETURNING_USER_KEY);
    ...
    // Check when user returns to the tab (from another tab/window)
  ...
  if (!showForm) {
    // Initial State - Compact, single viewport
    return (
      <section className="min-h-screen flex items-center justify-center relative overflow-hidden bg-background px-4 sm:px-6">
        {/* Floating Elements - Mix of Bitcoin & Mezo */}
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          <div
            className="absolute top-24 left-8 text-5xl opacity-12"
            style={{ transform: "rotate(-25deg)" }}
          >
            ₿
          </div>
          <div
            className="absolute bottom-24 right-6 opacity-12"
            style={{
          ...
          <div
            className="absolute top-50 right-8 opacity-12"
            style={{
        ...
        <div className="w-full max-w-md relative z-10">
          {/* Compact headline */}
          <h2 className="text-4xl sm:text-5xl font-medium tracking-tight mb-6 sm:mb-10 text-center">
          ...
          <div className="space-y-3 sm:space-y-4">
            <a
              ...
              className={cn(
                "block w-full h-12 sm:h-14 rounded-lg text-base sm:text-lg font-medium flex items-center justify-center gap-2 bg-foreground text-background border-2 border-transparent hover:scale-[1.02] transition-all duration-300",
                validationState === "valid" &&
                  "opacity-40 cursor-not-allowed pointer-events-none",
              )}
            >
              {dict.form.createMezoWallet}
              <ExternalLink className="h-5 w-5" />
            </a>

            {/* Small link to reveal form */}
            <button
              onClick={() => setShowForm(true)}
              className="w-full text-sm text-muted-foreground/80 hover:text-muted-foreground transition-colors text-center"
            >
              {dict.form.alreadyHaveWallet}{" "}
              <span className="text-gradient-mezo-cosmic">
                {dict.form.enterGiveaway} →
              </span>
            </button>
          </div>
        </div>
  ...
  // Expanded State - Form visible
  return (
    <section className="min-h-screen flex items-center justify-center relative overflow-hidden bg-background">
      {/* Floating Elements - Mix of Bitcoin & Mezo */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div
          className="absolute top-24 left-8 opacity-12"
          style={{ transform: "rotate(-25deg)", width: "80px", height: "80px" }}
        >
          <Image
            src="/Mezo Logo and Marks/Mezo Logo Circle.svg"
            alt=""
        ...
        <div
          className="absolute bottom-24 right-6 text-4xl opacity-12"
          style={{ transform: "rotate(40deg)" }}
        >
          ₿
        </div>
        <div
          className="absolute top-50 left-6 text-5xl opacity-12"
      ...
      <div className="container mx-auto px-4 sm:px-6 max-w-2xl relative z-10 py-4 sm:py-8">
        <div className="text-center mb-4 sm:mb-8">
          <h2 className="text-3xl sm:text-4xl font-medium tracking-tight mb-2 sm:mb-4">
            <span className="text-mezo-red">{dict.form.confirmYourEntry}</span>
          </h2>
        </div>

        <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-6">
          {/* Form Container - Compressed spacing */}
          <div className="bg-foreground/[0.02] backdrop-blur-sm rounded-3xl p-4 sm:p-6 space-y-3 sm:space-y-6 border border-border">
            {/* Wallet Address */}
            <div className="space-y-2">
              <Label
                htmlFor="wallet"
                className="text-xs uppercase tracking-wider text-muted-foreground/60 font-medium"
              >
              ...
              >
                <input
                  ...
                  className="w-full h-12 px-3 pr-12 text-base input-background border-0 rounded-md placeholder:text-muted-foreground focus:outline-none disabled:cursor-not-allowed disabled:opacity-50 [&:-webkit-autofill]:[-webkit-text-security:none]"
            ...
            <div className="pt-1 sm:pt-2">
              <Button
                ...
                className={cn(
                  "w-full h-12 sm:h-14 text-sm sm:text-base font-medium transition-all duration-300",
          ...
          <p className="text-center text-xs sm:text-sm text-muted-foreground/60">
            ...
            <a
              ...
              className={cn(
                "inline-block text-gradient-mezo-cosmic font-medium animate-bounce",
                validationState === "valid" &&
                  "opacity-40 cursor-not-allowed pointer-events-none",
              )}
            >
      ...
      <Dialog open={isSuccessOpen} onOpenChange={setIsSuccessOpen}>
        <DialogContent className="sm:max-w-lg border-2 border-border bg-background">
          <DialogHeader className="space-y-3 text-center">
            <DialogTitle className="text-2xl sm:text-4xl font-medium">
          ...
          {formData && (
            <div className="space-y-4 sm:space-y-8 py-3 sm:py-6">
              {/* Current Stats */}
              <div className="flex justify-center gap-8 sm:gap-16 py-4 sm:py-8 bg-gradient-to-br from-primary/5 to-primary/10 rounded-2xl">
                <div className="text-center">
                  <p className="text-[10px] sm:text-sm uppercase tracking-wider text-muted-foreground mb-1 sm:mb-3">
                    {dict.form.currentPosition}
                  </p>
                  <p className="text-3xl sm:text-5xl font-bold text-mezo-red">
                    #{formData.position}
                  </p>
                </div>
                <div className="text-center">
              ...
              <div className="space-y-2 sm:space-y-4">
                <div>
                  ...
                  <p className="text-xs sm:text-sm text-mezo-red">
              ...
              <div className="space-y-2 sm:space-y-4">
                ...
                <Button
                  ...
                  variant="outline"
                  className="w-full border-0 bg-foreground/5 hover:bg-foreground/10 h-10 sm:h-12 text-sm sm:text-base text-foreground"
                >
              ...
              <Button
                ...
                size="lg"
                className={cn(
                  "w-full h-12 sm:h-14 text-sm sm:text-base font-medium mt-4 sm:mt-8",
